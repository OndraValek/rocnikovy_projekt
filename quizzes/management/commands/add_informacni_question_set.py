...
class Command(BaseCommand):
    help = 'Přidá test Informační systémy a databázové systémy - Question Set s H5P standalone'

    def add_arguments(self, parser):
        parser.add_argument(
            '--h5p-path',
            type=str,
            help='Cesta k H5P souboru (např. h5p/ISaDS-question-set/) - pokud není zadáno, test se vytvoří bez h5p_path'
        )
    ...
    def handle(self, *args, **options):
        ...
        Quiz.objects.filter(topic=topic, title='Informační systémy a databázové systémy - Question Set').delete()
